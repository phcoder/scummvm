...
TextDisplayer_rpg::TextDisplayer_rpg(KyraRpgEngine *engine, Screen *scr) : _vm(engine), _screen(scr),
	...
	_waitButtonFont(Screen::FID_6_FNT), _isBig5(_vm->gameFlags().lang == Common::Language::ZH_TWN && _vm->game() == GI_EOB2) {
...
void TextDisplayer_rpg::printLine(char *str) {
	...
	if (sjisTextMode) {
		...
		if (_isBig5) {
			for (int i = 0; i < s; ++i) {
				if (str[i] & 0x80)
					twoByteCharOffs = Graphics::Big5Font::kChineseTraditionalWidth;
			}
		}
		...
		if ((lw + _textDimData[sdx].column) >= w) {
			...
			if (twoByteCharOffs) {
				...
				for (strPos = 0; strPos < s; ++strPos) {
					...
					if (_isBig5) {
						lw += Graphics::Big5Font::kChineseTraditionalWidth;
						strPos++;

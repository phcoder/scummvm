...
void Illusion::enterIllusion(int level) {
	...
	room->playAnimWithSpeech(Common::String::format("v7220bg%d", g_vm->getRnd().getRandomNumberRng(0, 5)),
				 TranscribedSound("v7220xc1",
						  _s("It's me, Phil. These beasts are all that stands between me and freedom")), 600, // unclear
...
void Illusion::handleEvent(int eventId) {
	...
	case 15306:
		...
		break;
	case 15307:
		room->playSpeech(TranscribedSound("v7220wg0", _s("Oh no, we're gonna fry")), 15308);
		break;
	case 15308:
		room->playSpeech(TranscribedSound("v7220wh0", _s("Let's get outta here")), 15309);
		break;
	case 15309:

...
DisplayedText Text::displayText(char *textPtr, uint32 bufLen, uint8 *dest, bool center, uint16 pixelWidth, uint8 color) {
	...
	int charHeight = isBig5 && _vm->_big5Font ? MAX<int>(_charHeight, _vm->_big5Font->getHeight()) : _charHeight;
	...
	do {
		...
		while (textChar >= 0x20) {
			if (isBig5 && (textChar & 0x80)) {
				...
				if (_vm->_big5Font->draw(curDest, lineEnd - curDest, charHeight, pixelWidth, fullCh, color, 240)) {
					//update position

...
bool SurfaceSdlGraphicsManager::loadGFXMode() {
	...
	_tmpscreen = SDL_CreateRGBSurface(SDL_SWSURFACE, _videoMode.screenWidth + _maxExtraPixels * 2,
						...
						_hwScreen->format->BytesPerPixel * 8,
	...
	if (actualDirtyRects > 0 || _cursorNeedsRedraw) {
		...
		for (r = _dirtyRectList; r != lastRect; ++r) {
			...
			if (dst_x < width && dst_y < height) {
				...
				_scaler->scale((byte *)srcSurf->pixels + (r->x + _maxExtraPixels) * srcSurf->format->BytesPerPixel + (r->y + _maxExtraPixels) * srcPitch, srcPitch,
					(byte *)_hwScreen->pixels + dst_x * _hwScreen->format->BytesPerPixel + dst_y * dstPitch, dstPitch, r->w, dst_h, r->x, r->y);
			}
...
void SurfaceSdlGraphicsManager::grabOverlay(Graphics::Surface &surface) const {
	...
	assert(surface.format.bytesPerPixel == _overlayscreen->format->BytesPerPixel);

	byte *src = (byte *)_overlayscreen->pixels;
	byte *dst = (byte *)surface.getPixels();
	Graphics::copyBlit(dst, src, surface.pitch, _overlayscreen->pitch,
		_videoMode.overlayWidth, _videoMode.overlayHeight, _overlayscreen->format->BytesPerPixel);
...
void SurfaceSdlGraphicsManager::blitCursor() {
	...
	// Used only if _cursorKeepPalettized is true.
	SDL_Color *targetPalette = _overlayVisible ? _overlayPalette : _currentPalette;

	if (_cursorKeepPalettized) {
		uint32 srcPitch = w * _mouseOrigSurface->format->BytesPerPixel;
		byte *dstPtr = (byte *)_mouseOrigSurface->pixels + _mouseOrigSurface->pitch * _maxExtraPixels + _maxExtraPixels * _mouseOrigSurface->format->BytesPerPixel;
		if (_cursorPaletteDisabled) {
			srcPtr = _mouseData;
			for (int i = 0; i < h; i++) {
				memcpy(dstPtr, srcPtr, srcPitch);
				memset(dstPtr + srcPitch, _mouseKeyColor, _mouseOrigSurface->pitch - srcPitch);
				srcPtr += srcPitch;
			...
			for (int i = 0; i < 256; i++)
				if (!mapping[i]) {
					_mappedMouseKeyColor = i;
					break;
				}
			mapping[_mouseKeyColor] = 0x100 | _mappedMouseKeyColor;
			srcPtr = _mouseData;
			for (int i = 0; i < h; i++) {
				for (int j = 0; j < w; j++)
					dstPtr[j] = mapping[srcPtr[j]];
				memset(dstPtr + srcPitch, _mappedMouseKeyColor, _mouseOrigSurface->pitch - srcPitch);
				srcPtr += srcPitch;
				dstPtr += _mouseOrigSurface->pitch;
			}
		}
		SDL_SetColors(_mouseOrigSurface, targetPalette, 0, 256);
	} else {
		byte *dstPtr;
	...
	if (sizeChanged || !_mouseSurface) {
		...
		if (_cursorKeepPalettized) {
			...
			_mouseSurface = SDL_CreateRGBSurface(SDL_SWSURFACE | SDL_RLEACCEL | SDL_SRCCOLORKEY | SDL_SRCALPHA,
							     _mouseCurState.rW,
							     _mouseCurState.rH,
							     8 * _hwScreen->format->BytesPerPixel,
							     _hwScreen->format->Rmask,
							     _hwScreen->format->Gmask,
							     _hwScreen->format->Bmask,
							     _hwScreen->format->Amask);
...
SDL_Surface *SurfaceSdlGraphicsManager::SDL_SetVideoMode(int width, int height, int bpp, Uint32 flags) {
	...
	_screenTexture = SDL_CreateTexture(_renderer, bpp == 8 ? SDL_PIXELFORMAT_RGB332 : SDL_PIXELFORMAT_RGB565, SDL_TEXTUREACCESS_STREAMING, width, height);
	if (!_screenTexture) {
		deinitializeRenderer();
		return nullptr;
	}
	...
	if (bpp == 8)
		...
		screen = SDL_CreateRGBSurface(0, width, height, 16, 0xF800, 0x7E0, 0x1F, 0);
	if (!screen) {
		deinitializeRenderer();
		return nullptr;
	} else {
